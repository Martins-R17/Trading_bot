...
set "RUN_LABEL=btc_15m_fast_futures_scalping_agents_%RUN_TS%"
set "OUTPUT_FILE=data\btc_15m_fast_futures_scalping_agents_%RUN_TS%.txt"

echo BTCUSDT 15m fast futures scalping agent search.
...
if "%EXIT_CODE%"=="0" (
  ...
  echo BTC 15m fast scalping search failed with exit code %EXIT_CODE%.
)
